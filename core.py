...
def build_stepwise_portfolio(universe_df):
    ...
    while True:
        ...
        logging.info(f"Building {step_label}")
        ...
        logging.info(f"US Exposure in {step_label}: {us_total_weight:.4f}")
        ...
        logging.info(f"Removing {len(us_names_to_remove)} US securities: {us_names_to_remove}")
...
logging.info("Stepwise portfolio exported to stepwise_portfolio_build2.xlsx")
